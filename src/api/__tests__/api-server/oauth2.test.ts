...
describe("ApiServer OAuth2", () => {
    ...
    describe("OAuth2 callback endpoint", () => {
        test("should handle successful OAuth2 callback", async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.text).toContain('id="threadloaf-oauth-data"');
            expect(response.text).toContain('data-jwt="mock-jwt-token"');
            expect(response.text).toContain('data-state="test-state"');
            expect(response.text).toContain('data-status="success"');
            expect(response.text).toContain("Logging into Threadloaf...");
        ...
        test("should handle OAuth errors gracefully", async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.text).toContain('id="threadloaf-oauth-data"');
            expect(response.text).toContain('data-error="Authentication failed"');
            expect(response.text).toContain('data-status="error"');
